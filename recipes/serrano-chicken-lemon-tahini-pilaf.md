---
title: Serrano-Spiced Chicken with Lemon-Tahini & Date-Almond Tapenade
slug: serrano-chicken-lemon-tahini-pilaf
tagline: Spice-rubbed chicken under a sweet-savory date-almond tapenade, drizzled with cool lemon-tahini.
pitch: >-
  This plate plays sweet, savory, nutty, and bright all at once. Chicken
  breasts get a paprika-garlic-serrano rub that perfumes the meat as it sears,
  then a chunky date-almond tapenade — pitted dates, toasted almonds, briny
  green olives, and capers chopped down with olive oil and lemon — spoons over
  the top for little hits of caramel, crunch, and salt. Garden peppers thread
  the whole dish: minced serrano in the rub, a little jalapeño in the tapenade,
  and sweet bell and banana pepper strips blistered on top. A cool, lemony
  tahini drizzle ties it together and cuts the richness.
  *The dates are the secret — their caramel sweetness against the salty olives
  and creamy tahini is what makes this taste like a restaurant dish.*
serves: 2
times:
  prep: 15
  cook: 20
  total: 35
difficulty: medium
protein: chicken
methods:
  - stove
  - oven
cuisine: Mediterranean
course: main
heat: medium
equipment:
  - cast iron
tags:
  - date-night
  - garden
  - meal-prep
photo: "Sliced seared chicken breast fanned on a plate, topped with a chunky date-and-almond tapenade (visible bits of chopped dates, toasted almonds, and green olives), drizzled with lemon-tahini, with blistered bell and banana pepper strips and diced cucumber alongside. No rice or grains on the plate."
ingredients:
  - section: Chicken & rub
    items:
      - 2 boneless skinless chicken breasts, pounded to even thickness
      - 1 tsp paprika
      - 1 serrano, seeded and minced (from the garden)
      - 1 garlic clove, minced
      - 1 tbsp olive oil
      - Kosher salt
  - section: Date-almond tapenade
    items:
      - ⅓ cup dates, pitted and finely chopped
      - ¼ cup almonds, toasted and chopped
      - ¼ cup green olives, pitted and chopped
      - 1 tbsp capers, drained and chopped
      - ½ jalapeño, seeded and minced (from the garden)
      - 1 garlic clove, minced
      - 2 tbsp parsley, chopped
      - 2 tbsp olive oil
      - Zest and juice of ½ lemon
      - Kosher salt and black pepper
  - section: Roasted peppers & finish
    items:
      - 1 bell pepper, sliced into strips
      - 1 banana pepper, sliced into strips
      - 1 tbsp olive oil
      - 3–4 tbsp lemon-tahini sauce (or tahini thinned with lemon juice)
      - ½ cucumber, diced
      - Lemon wedges
steps:
  - section: Prep & rub
    items:
      - Heat the oven to 425°F. Pound the chicken to an even thickness, then rub all over with paprika, salt, minced garlic, minced serrano, and olive oil. Let it sit at room temperature while you make the tapenade.
  - section: Make the tapenade
    items:
      - Pile the chopped dates, toasted almonds, olives, capers, jalapeño, garlic, and parsley on a cutting board and chop them together until chunky — you want distinct bits, not a smooth paste (or pulse briefly in a food processor).
      - Scrape into a bowl and stir in the olive oil, lemon zest and juice, and a pinch of salt and pepper. Let it sit so the flavors meld while you cook.
  - section: Roast the peppers
    items:
      - Toss the bell pepper and banana pepper strips with olive oil and salt on a sheet pan. Roast at 425°F until blistered and tender, about 10 minutes.
  - section: Sear the chicken
    items:
      - Heat a cast-iron skillet over medium-high. Sear the chicken 5–6 minutes per side until golden and 160°F internal; rest 5 minutes (carryover brings it to 165°F).
  - section: Finish & plate
    items:
      - Slice the chicken and fan it across the plates, then spoon the date-almond tapenade generously over and alongside.
      - Drizzle with lemon-tahini and garnish with the roasted peppers, diced cucumber, and a lemon wedge.
tips:
  - Mince the serrano fine and work it into the rub — it perfumes the meat from the inside as it sears without turning the dish into a chile challenge.
  - Pull the breasts at 160°F and rest them; carryover finishes them at 165°F so they stay juicy instead of drying out.
  - Toast the almonds in a dry pan until fragrant before chopping, and chop the tapenade by hand so you keep distinct bits of date, almond, and olive rather than a smooth paste — it only improves as it sits, so make it ahead.
  - If your tahini sauce is thick, loosen it with lemon juice and a splash of water until it drizzles in a ribbon.
extras:
  - label: From the garden
    note: Serrano in the rub, a little jalapeño in the tapenade, and sweet bell and banana peppers blistered on top — this dish is built for the pepper patch. Swap in whatever's ripe.
  - label: Starter
    note: Blister extra garden peppers and pile them over whipped goat cheese (mashed with olive oil, a splash of red wine vinegar, and salt) for a quick appetizer.
  - label: Pairs with
    note: A chilled Sauvignon Blanc or Albariño cuts the tahini richness and balances the heat.
source:
  name: Adapted from Stephen's ChatGPT recipe notes
  url: null
created: 2026-06-19
planSwaps:
  - for: [dash, mind, mediterranean, lowsugar, heart, balance]
    replace: "⅓ cup dates, pitted and finely chopped"
    with: "2 tbsp dates, pitted and finely chopped"
    note: "fewer dates"
  - for: [kidney]
    replace: "2 boneless skinless chicken breasts, pounded to even thickness"
    with: "2 boneless skinless chicken breasts (about 2½ oz each), pounded to even thickness"
    note: "a renal-size portion of chicken"
  - for: [kidney]
    replace: "¼ cup almonds, toasted and chopped"
    with: "2 tbsp almonds, toasted and chopped"
    note: "fewer almonds"
  - for: [kidney]
    replace: "3–4 tbsp lemon-tahini sauce (or tahini thinned with lemon juice)"
    with: "2 tbsp lemon-tahini sauce"
    note: "a lighter tahini drizzle"
updated: 2026-08-08
---

A pantry-raid dinner that turned into a keeper: a chunky date-and-almond tapenade
makes the plate taste Levantine, garden peppers thread heat and sweetness through
every layer, and a cool lemon-tahini drizzle pulls it all together.
